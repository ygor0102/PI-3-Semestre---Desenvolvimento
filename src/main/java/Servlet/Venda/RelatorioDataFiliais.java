...
public class RelatorioDataFiliais extends HttpServlet {
@Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
     ...
     try {
        ...
        if(usuario.isGerente()){
            List<ItemVenda> listaVendas03 = VendaDAO.reporteFiliaisData(Inicial, Final);
            request.setAttribute("listaVendas03", listaVendas03);
            
        }else{
            
            List<ItemVenda> listaVendas03 = VendaDAO.reporteFiliaisAdminData(Integer.parseInt(usuario.getFilial()), Inicial, Final);
            request.setAttribute("listaVendas03", listaVendas03);
        }
